...
u_int32_t p_table_0[128*128];
u_int32_t p_table_1[128*128];
    ...
    */
    for(int i = 0; i < 0x100; i++) {
        ...
        for(int j = 0; j < 8; j++) {
            ...
            u_int8_t pattern_byte = peek_vram(sprite_addr + j);
            u_int8_t pattern_byte_2 = peek_vram(sprite_addr + 8 + j);

            for(int k = 0; k < 8; k++) {

                u_int32_t* pixel = table_pointer + k + (j*128) + ((i%16)*8) + ((i/16) * 128 * 8); 
