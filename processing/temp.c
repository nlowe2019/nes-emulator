...
const u_int8_t NMI_BIT = 0b10000000;
const u_int8_t MASTER_SLAVE_BIT = 0b01000000;
const u_int8_t SPRITE_HEIGHT_BIT = 0b00100000;
const u_int8_t BACKGROUND_PATTERN_TABLE_BIT = 0b00010000;
const u_int8_t SPRITE_PATTERN_TABLE_BIT = 0b00001000;
const u_int8_t ADDR_INCREMENT_BIT = 0b00000100;
const u_int8_t NAME_TABLE_2_BIT = 0b00000010;
const u_int8_t NAME_TABLE_1_BIT = 0b00000001;
...
const u_int8_t GREYSCALE_BIT = 0b00000001;
const u_int8_t SHOW_BG_LEFT_BIT = 0b00000010;
const u_int8_t SHOW_SPRITES_LEFT_BIT = 0b00000100;
const u_int8_t SHOW_BG_BIT = 0b00001000;
const u_int8_t SHOW_SPRITES_BIT = 0b00010000;
const u_int8_t EMPHASIS_RED_BIT = 0b00100000;
const u_int8_t EMPHASIS_GREEN_BIT = 0b01000000;
const u_int8_t EMPHASIS_BLUE_BIT = 0b10000000;
...
const u_int8_t V_BLANK_BIT = 0b10000000;
const u_int8_t S0_HIT_BIT = 0b01000000;
const u_int8_t SPRITE_OVERFLOW_BIT = 0b00100000;
...
u_int8_t ppu_reg[0x8];
u_int8_t OAM_memory[0x100];
u_int8_t OAM_memory_secondary[0x20];

u_int32_t frame_buffer[240][256]; 
u_int8_t data_buffer = 0;

u_int16_t ppu_address_temp;  // 't' register
u_int16_t ppu_address;  // 'v' register
u_int8_t fine_x = 0;  // 'x' register 
...
u_int8_t sprite_priority[240][256];
...
u_int8_t read_ppu(u_int16_t addr) {

    u_int8_t return_data;
...
void write_ppu(u_int16_t addr, u_int8_t data) {
...
void Draw_Scanline() {
    ...
    if(dot == 257) {
        u_int16_t temp_copy = ppu_address_temp;
...
void Draw_Sprites() {
    ...
    u_int8_t sprite_y;
    u_int8_t pattern_index;
    u_int8_t attributes;
    u_int8_t sprite_x;
    u_int8_t sprite_lower;
    u_int8_t sprite_upper;
    SDL_Color color;
    ...
    for(int sprite_num = 0; sprite_num < 32; sprite_num+=4) {
        ...
        int pattern_table_addr = (ppu_reg[PPUCTRL] & SPRITE_PATTERN_TABLE_BIT) ? 0x1000 : 0x0000;
        ...
        u_int16_t palette_addr = 0x3f00 + 0x10 + (4 * palette_id);
...
void Draw_Nametable() {
    ...
    */
    ...
    int section = (dot/8) * 8;
//    int pattern_id = peek_vram(0x2000 | (ppu_address & 0x0fff));
    int pattern_id = peek_vram(0x2000 + (section) + (scanline*32));
    ...
    */
    ...
    u_int8_t pattern_byte = peek_vram(pattern_addr);
    u_int8_t pattern_byte_2 = peek_vram(pattern_addr + 8);
    ...
    for(int i = 0; i < 8; i++) { 
        ...
        */
        ...
        //u_int16_t attr_addr = 0x23c0 | (ppu_address & 0x0c00) | ((ppu_address >> 4) & 0x38) | ((ppu_address >> 2) & 0x07);
        u_int16_t attr_addr = 0x23c0 + (section/8) + (scanline/8);
        u_int8_t palette_id = read_vram(attr_addr);
        palette_id = (palette_id >> (2 * quadrant)) & 0b11;
        u_int16_t palette_addr = 0x3f00 + (4*palette_id);
    
        u_int8_t value = (pattern_byte >> (7-(i%8))) & 0b1;
        ...
        if(sprite_priority[scanline][section+i] == 0) {
...
void OAM_DMA(u_int8_t data) {
    ...
    u_int16_t ram_addr = 0x100 * data;
    u_int8_t oam_addr = ppu_reg[OAMADDR];
