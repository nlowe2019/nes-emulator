    ...
    switch(addr) {
        ...
        case 0x06:
            if(latch == 0) {
                ...
                ppu_address_temp = (ppu_address_temp & 0xff) + (ppu_reg[PPUADDR] << 8);
                latch = 1;
            } else {
                ppu_reg[PPUADDR] = data;
                ppu_address_temp = (ppu_address_temp & 0xff00) + ppu_reg[PPUADDR];
...
void Draw_Nametable() {
    ...
    */
    ...
    int section = (dot/8) * 8;
    int pattern_id = peek_vram(0x2000 + (0x400 * (NAME_TABLE_ID)) + ((scanline/8)*32) + (section/8));
    ...
    */
    ...
    for(int i = 0; i < 8; i++) { 
        ...
        */
        ...
        u_int16_t attr_addr = 0x23c0 + palette_tile_x + (8 * palette_tile_y);
