...
u_int16_t pc = 0;
u_int8_t sp = 0xfd;
u_int8_t status = 0b100;

u_int8_t accumulator = 0;
u_int8_t index_x = 0;
u_int8_t index_y = 0;
...
int Update_CPU() {
    ...
    u_int8_t opcode = FetchInstruction();
...
u_int8_t FetchInstruction() {

    u_int8_t opcode = read(pc);
    pc ++;
    return opcode;
}

opcode OpcodeLookup(u_int8_t opcode) {
...
int ExecuteInstruction(opcode opcode) {
    ...
    u_int16_t addr = GetAddress(mode, &page_cross);
...
/*
    if (mode == IMMEDIATE || mode == ZERO_PAGE || mode == ZERO_PAGE_X || mode == INDIRECT_X || mode == INDIRECT_Y || mode == ZERO_PAGE_Y || mode == RELATIVE ) {
        printf("[%04x] %s: %s (%02x), %02x\n", pc-2, get_address_mode_string((u_int8_t)mode), get_opcode_name(opcode.OPCODE), instruction, read(pc-1));
  //      fprintf(file_ptr, "\n%04x  %02x %02x     %s  %04x", pc-2, read(pc-2), read(pc-1), get_opcode_name(opcode.OPCODE), addr);
    }
    else if (mode == IMPLIED || mode == ACCUMULATOR ) {
        printf("[%04x] %s: %s (%02x)\n", pc-1, get_address_mode_string((u_int8_t)mode), get_opcode_name(opcode.OPCODE), instruction);
    //    fprintf(file_ptr, "\n%04x  %02x        %s  %04x", pc-1, read(pc-1), get_opcode_name(opcode.OPCODE), addr);
    }
    else {
        printf("[%04x] %s: %s (%02x), %02x %02x\n", pc-3, get_address_mode_string((u_int8_t)mode), get_opcode_name(opcode.OPCODE), instruction, read(pc-1), read(pc-2));
...
u_int16_t GetAddress(enum ADDRESS_MODE mode, int* page_cross) {
    ...
    u_int16_t addr = 0x0000;
    u_int16_t ptr = 0x0000;
...
int adc6502(enum ADDRESS_MODE mode, u_int16_t addr) {
    ...
    u_int8_t accumulator_initial = accumulator;
    u_int8_t value = (mode == IMMEDIATE) ? addr : read(addr);

    u_int16_t result = accumulator + value + (status & FLAG_CARRY ? 1 : 0);
...
int asl6502(enum ADDRESS_MODE mode, u_int16_t addr) {
    ...
    u_int8_t value = (mode == ACCUMULATOR) ? accumulator : read(addr);
...
int rol6502(enum ADDRESS_MODE mode, u_int16_t addr) {
    ...
    u_int8_t carry = (status & FLAG_CARRY ? 1 : 0);
...
int ror6502(enum ADDRESS_MODE mode, u_int16_t addr) {
    ...
    u_int8_t carry = (status & FLAG_CARRY ? 0b10000000 : 0);
...
char* get_address_mode_string(u_int8_t address_num) {
...
char* get_opcode_name(u_int8_t op) {
